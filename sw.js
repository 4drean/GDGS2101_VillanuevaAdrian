/*self.addEventListener("fetch",evento=>{
    const sinConexion=fetch("/pages/sinconexion.html")
  
    const respuesta=fetch(evento.request)
        .catch(()=>{
        return sinConexion();
    });
    evento.respondingWith(respuesta);
});*/
...
self.addEventListener("fetch",event=>{
    ...
    const respuesta = caches.open(CACHE_STATIC_NAME)
        .then(caches=>{
            fetch(event.request).then(nuevaRespuest=>{
                cache.put(event.request,nuevaRespuest);
    ...
    event.respondWith(caches.match(event.request)
        .then(resp=>{
            if(resp) return resp;
            console.log("no existe el cache",event.request.url);
